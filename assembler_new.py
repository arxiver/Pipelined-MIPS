...
def convert_assembly_binary(assembilylines):       
    ...
    for line in assembilylines:
        ...
        if ins_type == 0:                         
            ...
            Opcode_assembly_out.append(decimal2binary(int(line),16))
...
def two_operand_assmebly_binary_convert(line):
    ...
    if(Opcode.upper()=="SWAP"):
        ...
        b_src,b_dst,b_imm=Assembly_opcode_map[src],Assembly_opcode_map[dst],decimal2binary(int(imm),16)
...
def memory_operand_assmebly_binary_convert_private(line):
    ...
    if(OpCode == "LDM"):
        Imm = decimal2binary(int(elements[2]),16)
        IR[14:30] = list(Imm)
        return IR

    EA = decimal2binary(int(elements[2]),20)
    IR[11:31] = EA
    return IR
...
if __name__ == "__main__":  
    ...
    Arr[Org0][0] = str(int(Arr[Org0][0])-1)
    Arr[Org2][0] = str(int(Arr[Org2][0])-1)
